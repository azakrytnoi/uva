	...
	public:
		...
		void solve(int level = 0) {
			if (level == tracks_.size()) {
				size_t idx(0);
				int64_t running = std::accumulate(tracks_.begin(), tracks_.end(), int64_t(0), [&](int64_t init, auto val) -> int64_t { return init + (current_[idx++] ? val : 0); });
