...
namespace
{
	...
	public:
		...
		operator bool() const { return !summ_.empty(); }
	...
	std::istream& operator >> (std::istream& in, solution& sol)
	{
		...
		if (in >> n)
		{
			...
			std::generate_n(std::back_inserter(sol.summ_), limit, [&]() { int32_t tmp; in >> tmp; return tmp; });
		}
		return in;
	}
	...
	bool solution::calculate(size_t start, std::vector<bool> taken)
	{
		...
		for (size_t i = 2; i < summ_.size(); i++)
		{
			if (start == 2)
			{
				...
				if (std::fabs(candidate - int32_t(candidate)) > 0.00000001) continue;
			...
			else
			{
				...
				if (taken[i]) continue;
				if (summ_[i] - result_.back() != result_[start % pivot]) continue;
				taken[i] = true;
			}

			if (calculate(start + 1, taken)) return true;

			taken[i] = false;
			if (start == 2) result_.clear();
			if (start == pivot) result_.pop_back();
		}
