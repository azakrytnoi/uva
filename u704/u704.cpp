...
namespace
{
		...
		std::queue<wheels>& working_q)
	{
		...
		switch (direction) {
		case rotation::left_clockwise:
			new_dir = rotation::left_counterclockwise; break;
		case rotation::right_clockwise:
			new_dir = rotation::right_counterclockwise; break;
		case rotation::left_counterclockwise:
			new_dir = rotation::left_clockwise; break;
		case rotation::right_counterclockwise:
			new_dir = rotation::right_clockwise; break;
		}
	...
	std::istream& operator >> (std::istream& in, solution& sol)
	{
		...
		for (size_t i = 0; i < numbers_per_wheel; i++) {
			std::cin >> nr;
			sol.wheels_.first <<= 4; sol.wheels_.first |= nr;
		}
		for (size_t i = 0; i < numbers_per_wheel; i++) {
			std::cin >> nr;
			sol.wheels_.second <<= 4; sol.wheels_.second |= nr;
		}
		return in;
	}
