...
void U558::operator()()
{
	...
	while (N--)
	{
		...
		std::generate_n(std::back_inserter(wormholes), n_wormholes, []() { edge e; std::cin >> e.start >> e.end >> e.weight; return e; });

		distances.reserve(n_systems + 1);
		std::generate_n(std::back_inserter(distances), n_systems, []() { return U558::Infinity; });
		distances[0] = 0;
