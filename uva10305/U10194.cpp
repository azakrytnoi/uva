...
namespace {
	...
	public:
		...
		int games_played() const { return wins_ + losses_ + ties_; }
		int& wins() { return wins_; }
		int& ties() { return ties_; }
		int& losses() { return losses_; }
		int goals() const { return goals_scored_ - goals_against_; }
		int& goals_scored() { return goals_scored_; }
		int& goals_against() { return goals_against_; }
		const std::string& name() const { return name_; }

		void win() { wins_++;  points_ += 3; }
		void loss() { losses_++; }
		void tie() { ties_++; points_ += 1; }
...
void U10194::operator()()
{
	...
	while (N--)
	{
		...
		teams.clear();
		std::getline(std::cin, line);
		std::generate_n(std::back_inserter(results), n, [&]() { std::getline(std::cin, line); auto t = std::make_shared<team>(line); teams[line] = t; return t; });
		std::cin >> n;
		std::getline(std::cin, line);
		while (n--)
		{
		...
		std::sort(results.begin(), results.end(), [](auto t1, auto t2) { return *t2 < *t1; });
		int position = 0;
		std::for_each(results.begin(), results.end(), [&position](auto t) { std::cout << (++position) << ") " << *t << std::endl; });
		std::cout << std::endl;
	}
}
