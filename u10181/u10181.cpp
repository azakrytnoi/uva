...
namespace
{
	...
	int solution::dfs(int pos, int depth, int8_t d)
	{
		...
		if (depth + h > bound_) return depth + h;
		if (h == 0)
		{
			solved_ = true;
			return depth;
		...
		for (int i = 0; i < DIR_N; ++i)
		{
			...
			if (0 <= nx && nx < SIZE && 0 <= ny && ny < SIZE)
			{
				path_[depth] = i;
				int npos = nx * SIZE + ny;
				cur_[pos] = cur_[npos]; cur_[npos] = 0;     /* pos -> npos */
				int t = dfs(npos, depth + 1, i);
				if (solved_) return t;
				nbound = std::min(nbound, t);
				cur_[npos] = cur_[pos]; cur_[pos] = 0;
			}
		}
		return nbound;
	}
	...
	solution& solution::operator()()
	{
		...
		{
			path_.clear();
			std::generate_n(std::back_inserter(path_), MAX_DEPTH, []() {return -1; });
			IDAstar();
		}
		return *this;
	}
