...
namespace
{
	...
	std::istream& operator >> (std::istream& in, solution& sol)
	{
		...
		if (in) {
			std::generate_n(std::back_inserter(sol.fares_), 24, [&]() { uint16_t tmp; in >> tmp; return tmp; });
			std::string line;
			std::getline(in, line);
	...
	solution::operator bool()
	{
		...
		{
			trip->second.first = 0;
			std::sort(trip->second.second.begin(), trip->second.second.end(), [](trip_history& l, trip_history& r) { return l.day_ * 24 * 60 + l.hour_ * 60 + l.minute_ < r.day_ * 24 * 60 + r.hour_ * 60 + r.minute_; });
